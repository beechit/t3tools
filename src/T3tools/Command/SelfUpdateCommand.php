 ...
 *
 * Copy form https://github.com/padraic/humbug/blob/master/src/Command/SelfUpdate.php
 */
 ...
 *
 * @package BeechIt\T3tools\Command
 */
class SelfUpdateCommand extends Command
{

    const VERSION_URL = 'https://beechit.github.io/t3tools/downloads/t3tools.version';

    const PHAR_URL = 'https://beechit.github.io/t3tools/downloads/t3tools.phar';

    const PACKAGE_NAME = 'beechit/t3tools';

    const FILE_NAME = 't3tools.phar';
    ...
    protected function update(Updater $updater)
    {
        ...
        try {
            ...
            if ($result) {
                $this->output->writeln('<fg=green>T3tools has been updated.</fg=green>');
                ...
                $this->output->writeln('<fg=green>T3tools is currently up to date.</fg=green>');
                $this->output->writeln(sprintf(
                    '<fg=green>Current version is:</fg=green> <options=bold>%s</options=bold>.',
                    $oldVersion
                ));
    ...
    protected function rollback()
    {
        ...
        try {
            ...
            if ($result) {
                $this->output->writeln('<fg=green>T3tools has been rolled back to prior version.</fg=green>');
            } else {
    ...
    protected function configure()
    {
        $this
            ->setName('self-update')
            ->setDescription('Update t3tools.phar to most recent stable, pre-release or development build.')
            ->addOption(
                'dev',
                'd',
                InputOption::VALUE_NONE,
                'Update to most recent development build of T3tools.'
            )
            ->addOption(
                'non-dev',
                'N',
                InputOption::VALUE_NONE,
                'Update to most recent non-development (alpha/beta/stable) build of T3tools tagged on Github.'
            )
            ->addOption(
                'pre',
                'p',
                InputOption::VALUE_NONE,
                'Update to most recent pre-release version of T3tools (alpha/beta/rc) tagged on Github.'
            )
            ->addOption(
                'stable',
                's',
            ...
            ->addOption(
                ...
                'r',
                InputOption::VALUE_NONE,
                'Rollback to previous version of T3tools if available on filesystem.'
            )
            ->addOption(
                'check',
                'c',
